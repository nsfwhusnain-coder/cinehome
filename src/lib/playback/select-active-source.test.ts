...
describe("selectActiveSource", () => {
  it("Ultra starts remux 4K when that is the only 4K, even after 1080 has begun", () => {
    ...
    const upgraded = selectActiveSource({
      roster: [remux4k, direct1080],
      active: direct1080,
      userPicked: false,
      everPlayed: true,
    ...
    expect(upgraded.replace).toBe(true);
    expect(upgraded.next?.id).toBe("remux-4k");
  });
