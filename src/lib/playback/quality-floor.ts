/**
 * Household picture-quality floor.
 *
 * A labelled 1080p at 2 Mbps is a CAM/WEBRip smear, not HD. These numbers are
 * H.264-equivalent. Unknown bitrate is not treated as lean — absence of a
 * manifest rate is not evidence.
 *
 * Keep in sync with mini-services/stream-scraper/quality-floor.ts.
 */

export const QUALITY_FLOOR_HEIGHT = 720;

/** Minimum declared bitrate (bps) that still counts as the claimed height. */
...
/**
 * Drop capture / lean / sub-HD rows when a better alternative exists.
 * Never empties the roster — a cam is still better than no stream.
 */
