...
function AllDebridSection() {
  const qc = useQueryClient();
  const [token, setToken] = useState("");

  const { data: status, isLoading, isFetching, refetch } = useQuery<RealDebridStatusData>({
    queryKey: ["alldebrid-status"],
    queryFn: async () => {
      const res = await fetch("/api/debrid/alldebrid/status");
      if (!res.ok) throw new Error("Failed to load status");
      return res.json();
    },
  });
  ...
  const applyStatus = (next: RealDebridStatusData) => {
    qc.setQueryData(["alldebrid-status"], next);
    qc.invalidateQueries({ queryKey: ["alldebrid-status"] });
  };
  ...
  const save = useMutation({
    ...
    onSuccess: (next) => {
      toast.success(next.premium ? "AllDebrid connected — premium active" : "API key saved");
      setToken("");
      applyStatus(next);
    },
    onError: (err) => toast.error(err instanceof Error ? err.message : "Failed to save API key"),
  });

  const remove = useMutation({
    mutationFn: async () => {
      const res = await fetch("/api/debrid/alldebrid/status", { method: "DELETE" });
      const json = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(json?.error || "Failed to remove key");
      return json as RealDebridStatusData;
    },
    onSuccess: (next) => {
      toast.success("Removed saved AllDebrid key");
      applyStatus(next);
    },
    onError: (err) => toast.error(err instanceof Error ? err.message : "Failed to remove key"),
  });
  ...
  return (
    <Card className="rounded-2xl">
      <CardHeader className="flex flex-row items-start justify-between gap-2 space-y-0">
        <div>
          <CardTitle className="font-display flex items-center gap-2 text-base">
            <Crown className="h-4 w-4 text-sky-400" /> AllDebrid
          </CardTitle>
          <CardDescription>
            Paste a new API key to activate it live — no restart, no .env edit.
          </CardDescription>
        </div>
        <Button
          variant="ghost"
          ...
          disabled={isFetching}
          className="rounded-full"
          aria-label="Refresh AllDebrid status"
        >
          <RefreshCw className={`h-4 w-4 ${isFetching ? "animate-spin" : ""}`} />
        </Button>
      </CardHeader>
        ...
        {isLoading || !status ? (
          ...
          <>
            <div className="flex items-center justify-between">
              <span className="text-sm">Subscription</span>
              {active ? (
                <Badge className="bg-sky-500/15 text-sky-400 hover:bg-sky-500/20">
                  <CheckCircle2 className="mr-1 h-3 w-3" /> Active · Premium
                </Badge>
              ) : status.configured ? (
                <Badge variant="secondary">
                  <AlertTriangle className="mr-1 h-3 w-3" />
                  {status.valid ? "Not premium" : "Key invalid"}
                </Badge>
              ) : (
                <Badge variant="secondary">
                  <XCircle className="mr-1 h-3 w-3" /> Not connected
            ...
            {active ? (
              <div className="grid grid-cols-2 gap-3 rounded-xl border border-sky-500/15 bg-sky-500/[0.04] p-3 text-sm">
                <div>
                  <div className="text-xs text-muted-foreground">Account</div>
                  <div className="font-medium">{status.username ?? "—"}</div>
                </div>
              ...
              <p className="text-sm text-muted-foreground">
                No key yet — paste your AllDebrid API key below. Saving updates the live
            ...
            <div className="space-y-1.5">
              <Label htmlFor="ad-token">{status.configured ? "Update API key" : "API key"}</Label>
              <Input
                id="ad-token"
                type="password"
                autoComplete="off"
              ...
              />
              <p className="text-xs text-muted-foreground">
                Get it from <code className="rounded bg-muted px-1">alldebrid.com/apikeys</code>.
                Validated before saving; never shown again.
              </p>
            </div>
